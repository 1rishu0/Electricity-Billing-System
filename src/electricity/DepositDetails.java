...
public class DepositDetails extends JFrame implements ActionListener {
    ...
    DepositDetails() {
        ...
        try {
            ...
            // Fetching meter numbers
            String s2 = "select meter from customer";
            ResultSet rs = c.s.executeQuery(s2);
            while(rs.next()) {
                c1.add(rs.getString("meter"));
            }
            
            // Initially populate the table with all bills
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        if(ae.getSource() == b1) {
            String s = "select * from bill where meter='" + c1.getSelectedItem() + "' and month='" + c2.getSelectedItem() + "'";
